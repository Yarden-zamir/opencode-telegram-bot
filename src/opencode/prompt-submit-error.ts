...
function asObject(value: unknown): PlainObject | null {
  return value && typeof value === "object" ? (value as PlainObject) : null;
}
...
function collectErrorTexts(error: unknown): string {
  ...
  if (Array.isArray(root.errors)) {
    for (const item of root.errors) {
      ...
      const itemMessage = itemObject
        ? readString(itemObject.message) ?? readString(itemObject.error)
        : null;
      if (itemMessage) {
...
function looksBusyText(text: string): boolean {
  return /\b(busy|already running|currently running|still running|in progress|another run|another task|please wait)\b/i.test(
    text,
  );
}
